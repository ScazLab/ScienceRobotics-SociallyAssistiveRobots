...
namespace TrainGame
{
	...
	public delegate void ReceivedMessageEventHandler(object sender, int game, int command, int level);
	...
	public class RosbridgeWebSocketClient
	{
		...
		void HandleOnMessage (object sender, MessageEventArgs e)
		{
			...
			int command = -1;
			int level = -1;
			RosbridgeUtilities.DecodeROSJsonCommand(e.Data, out game, out command, out level);
			...
			if(this.receivedMsgEvent != null) {
				...
				this.receivedMsgEvent(this, game, command, level);
			}
		}
